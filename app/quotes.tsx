...
export default function QuotesScreen() {
  ...
  const confirmAddPanel = () => {
    ...
    const panelNumber = selectedPanelForAdd;
    const category = selectedCategoryForAdd;
    const price = panelPricing.pricing[panelNumber]?.[category] ?? 0;
        ...
        <View style={styles.panelModalOverlay}>
          <View style={styles.panelModalContent}>
            <View style={styles.panelModalHeader}>
              <Text style={styles.panelModalTitle}>
                {selectedPanelForAdd !== null ? 'Select Category' : 'Select Panel'}
              </Text>
              <TouchableOpacity onPress={() => {
                setAddPanelModalVisible(false);
                setSelectedPanelForAdd(null);
            ...
            {selectedPanelForAdd !== null && panelPricing ? (
              ...
              <View style={styles.categorySelectionView}>
                ...
                <View style={styles.categoryGrid}>
                  {CATEGORIES.map((cat) => {
                    const catPrice = panelPricing.pricing[selectedPanelForAdd]?.[cat];
                    const isManual = catPrice === null || catPrice === undefined;
                    const displayPrice = isManual ? 'Manual' : `$${catPrice}`;
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        style={[
                          ...
                          selectedCategoryForAdd === cat && styles.categoryGridItemActive,
                        ]}
                        onPress={() => setSelectedCategoryForAdd(cat)}
                      >
                        <Text style={[
                          styles.categoryGridNumber,
                          selectedCategoryForAdd === cat && styles.categoryGridNumberActive,
                        ]}>
                          Cat {cat}
                        </Text>
                        <Text style={[
                          styles.categoryGridPrice,
                          selectedCategoryForAdd === cat && styles.categoryGridPriceActive,
                          isManual && styles.categoryGridPriceManual,
                        ]}>
                          {displayPrice}
                        </Text>
                      </TouchableOpacity>
                ...
                <View style={styles.categoryActionButtons}>
                  <TouchableOpacity 
                    ...
                    onPress={() => setSelectedPanelForAdd(null)}
                  >
                  ...
                  <TouchableOpacity 
                    ...
                    onPress={confirmAddPanel}
                  >
                    <Ionicons name="checkmark" size={20} color="#FFFFFF" />
                    <Text style={styles.categoryConfirmText}>Add Panel</Text>
                  </TouchableOpacity>
                </View>
              </View>
            ) : (
...
const styles = StyleSheet.create({
  ...
  categorySelectionView: { padding: 20 },
  selectedPanelInfo: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 24, backgroundColor: '#F3F4F6', padding: 16, borderRadius: 12 },
  panelBadgeLarge: { width: 48, height: 48, borderRadius: 24, backgroundColor: '#3B82F6', justifyContent: 'center', alignItems: 'center' },
  panelNumberLarge: { fontSize: 18, fontWeight: '700', color: '#FFFFFF' },
  selectedPanelName: { flex: 1, fontSize: 18, fontWeight: '600', color: '#1F2937' },
  selectCategoryLabel: { fontSize: 14, fontWeight: '600', color: '#6B7280', marginBottom: 12 },
  categoryGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 24 },
  categoryGridItem: { width: '18%', aspectRatio: 1, backgroundColor: '#F3F4F6', borderRadius: 12, justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: 'transparent' },
  categoryGridItemActive: { backgroundColor: '#EFF6FF', borderColor: '#3B82F6' },
  categoryGridNumber: { fontSize: 14, fontWeight: '700', color: '#6B7280' },
  categoryGridNumberActive: { color: '#3B82F6' },
  categoryGridPrice: { fontSize: 11, color: '#9CA3AF', marginTop: 2 },
  categoryGridPriceActive: { color: '#3B82F6' },
  categoryGridPriceManual: { color: '#F59E0B' },
  categoryActionButtons: { flexDirection: 'row', gap: 12 },
  categoryBackButton: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 14, backgroundColor: '#F3F4F6', borderRadius: 12 },
  categoryBackText: { fontSize: 15, fontWeight: '600', color: '#6B7280' },
  categoryConfirmButton: { flex: 2, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 14, backgroundColor: '#3B82F6', borderRadius: 12 },
  categoryConfirmText: { fontSize: 15, fontWeight: '600', color: '#FFFFFF' },
